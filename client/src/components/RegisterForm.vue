    ...
    >
      <div class="mb-4">
        ...
        <input
          ...
          id="username"
        />
        <h5 id="usernameHelp" class="form-text text-muted" v-if="errorsMsg.length">
        ...
        <input
          ...
          id="password"
        />
        <h5 id="passwordHelp" class="form-text text-muted" v-if="errorsMsg.length">
        ...
        <input
          ...
          id="password2"
        />
        <h5 id="confirmPasswordHelp" class="form-text text-muted" v-if="errorsMsg.length">
          Please confirm your password.
        </h5>
      </div>
      <div v-if="errorsMsg" class="flex items-center mb-4" role="alert">
        <div
      ...
      <div class="flex items-center justify-between">
        ...
        />
        <div class="lds-grid" v-if="signingUp"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>
...
const url = process.env.PORT || "http://localhost:3000/auth/signup"
      ...
      if (this.validUser()) {
        ...
        this.$http({
          ...
          .then(data => {
            ...
            setTimeout(() => {
              console.log(data)
              // if register successful redirect
              this.$router.push('/login')
            }, 1000)

          })
          .catch(err => this.errorsMsg = err.message)
      }
    },
    validUser() {
      if (this.user.password !== this.user.password2) {
        this.errorsMsg = "Passwords do not match"
        return false
      }
      ...
      if (result.error.message.includes("username")) {
        this.errorsMsg = "User is invalid 😭"
      } else {
        this.errorsMsg = "Password is invalid"
      }
